...
def create_route():
    ...
    try:
        ...
        # Validación
        if not data or not data.get('country') or not data.get('city') or not data.get('points_of_interest'):
            return jsonify({"message": "Faltan datos: country, city, points_of_interest"}), 400
        ...
        new_route = Route(
            ...
            points_of_interest=json.dumps(data['points_of_interest']) if isinstance(data['points_of_interest'], list) else data['points_of_interest'],
            coordinates=json.dumps(data['coordinates']) if data.get('coordinates') else None
        )
...
def update_route(route_id):
    ...
    try:
        ...
        if data.get('points_of_interest'):
            route.points_of_interest = json.dumps(data['points_of_interest']) if isinstance(data['points_of_interest'], list) else data['points_of_interest']
...
def delete_route(route_id):
    ...
    try:
        ...
        if route.user_id != user_id and user.role != UserRole.ADMIN:
            return jsonify({"message": "No tienes permisos para eliminar esta ruta"}), 403
...
def get_routes_by_city(city):
    ...
    try:
        routes = Route.query.filter_by(city=city).order_by(Route.created_at.desc()).all()
        return jsonify([route.serialize() for route in routes]), 200
    except Exception as e:
...
def geocode_location(location):
    ...
    try:
        ...
        headers = {
            'User-Agent': 'TravelRoutesApp/1.0'  # Nominatim requiere User-Agent
        }
