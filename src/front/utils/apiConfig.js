...
// If VITE_BACKEND_URL is undefined or empty string, use empty string for relative paths
const backendUrl = import.meta.env.VITE_BACKEND_URL;
export const API_BASE_URL = backendUrl === undefined || backendUrl === '' ? '' : backendUrl;
...
const getFromCache = (key) => {
    const cached = cache.locations.get(key) || cache.pois.get(key) || cache.images.get(key);
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 403) {
                ...
                // Segundo intento
                const retryResponse = await fetch(`https://nominatim.openstreetmap.org/search?${params}`, {
                    headers: {
...
`;
    ...
    for (let serverIndex = 0; serverIndex < overpassServers.length; serverIndex++) {
        ...
        try {
            console.log(`🌐 Intentando servidor ${serverIndex + 1}/${overpassServers.length}: ${server}`);
            ...
            if (response.status === 429) {
                console.warn(`⚠️ Rate limit en servidor ${serverIndex + 1}, probando siguiente...`);
            ...
            const pois = data.elements
                ...
                .map((el) => {
                    ...
                    return {
                        id: el.id,
                        name: el.tags.name,
                        type: type, // Use the category type instead of OSM tag value
                        lat: poiLat,
                        lon: poiLon,
            ...
            if (error.name === 'AbortError') {
                ...
                console.error(`❌ Error en servidor ${serverIndex + 1}:`, error.message);
            }
